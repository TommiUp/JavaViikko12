...
public class InfoListAdapter extends RecyclerView.Adapter<ProductViewHolder> {
    private Context context;
    private ArrayList<Product> products;

    private Storage storage = Storage.getInstance();

    public InfoListAdapter(Context context, ArrayList<Product> products) {
        ...
        holder.editImage.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (holder.editName.getVisibility() == view.VISIBLE){
                    Product product = Storage.getInstance().getImportantProductByIdWithoutRemove(pos);
    ...
    public void removeProduct(Product product) {
        ...
        if (position != -1) {
            if (Storage.getInstance().getProducts().contains(product)){
                Storage.getInstance().getProducts().remove(product);
            }
